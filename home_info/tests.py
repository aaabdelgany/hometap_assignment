...
class Tests(TestCase):
    ...
    def test_3(self):
        #test to local septic_check route with the wrong HTTP method to verify that the status code is 405, forbidden
        url=reverse('septic_check')
        resp = client.get(url)
        print(resp)
        self.assertEqual(resp.status_code,405)
    def test_4(self):
        #test to the local septic_check route with the right HTTP method, the request will fail because the util is missing the necessary
        #credentials
